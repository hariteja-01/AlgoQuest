import React, { useState, useCallback, useEffect, useRef, useMemo } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Play, Pause, SkipForward, RotateCcw, Sparkles, Zap, Navigation } from 'lucide-react';
import { useApp } from '../context/AppContext';
import CodeDisplay from '../components/CodeDisplay';
import { DijkstraEngine, DijkstraStep } from '../logic/dijkstra/engine';
import { generateDijkstraCode } from '../logic/dijkstra/codegen';

const PRESETS = [
  { key: 'simple' as const, label: 'Simple' },
  { key: 'dense' as const, label: 'Dense' },
  { key: 'sparse' as const, label: 'Chain' },
];

const Dijkstra: React.FC = () => {
  const { theme, language } = useApp();
  const dark = theme === 'dark';

  const [config, setConfig] = useState(() => DijkstraEngine.generate('simple'));
  const [steps, setSteps] = useState<DijkstraStep[]>([]);
  const [stepIdx, setStepIdx] = useState(0);
  const [playing, setPlaying] = useState(false);
  const [speed, setSpeed] = useState(700);
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  const run = useCallback(() => {
    const engine = new DijkstraEngine();
    setSteps(engine.solve(config.nodes, config.edges, config.source));
    setStepIdx(0);
    setPlaying(false);
  }, [config]);

  useEffect(() => { run(); }, [run]);

  useEffect(() => {
    if (playing && stepIdx < steps.length - 1) {
      timerRef.current = setTimeout(() => setStepIdx(i => i + 1), speed);
    } else if (stepIdx >= steps.length - 1) setPlaying(false);
    return () => { if (timerRef.current) clearTimeout(timerRef.current); };
  }, [playing, stepIdx, steps.length, speed]);

  const current = steps[stepIdx] || steps[0];
  const code = generateDijkstraCode(config.nodes, config.edges, config.source, language);

  const nodePositions = useMemo(() => {
    const n = config.nodes;
    const cx = 170, cy = 150, r = 120;
    return Array.from({ length: n }, (_, i) => {
      const angle = (i / n) * Math.PI * 2 - Math.PI / 2;
      return { x: cx + Math.cos(angle) * r, y: cy + Math.sin(angle) * r };
    });
  }, [config.nodes]);

  return (
    <div className={`min-h-screen px-4 py-6 sm:px-6 lg:px-8 ${dark ? 'bg-slate-950' : 'bg-slate-50'}`}>
      <div className="mx-auto max-w-7xl flex flex-col gap-5">
        <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}
          className={`rounded-3xl border p-5 shadow-xl backdrop-blur ${dark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-white/85'}`}>
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="space-y-1">
              <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-teal-400">
                <Navigation className="h-4 w-4" /> Dijkstra's Algorithm
              </div>
              <h1 className={`text-2xl font-black tracking-tight ${dark ? 'text-white' : 'text-slate-900'}`}>
                Shortest Path Explorer
              </h1>
              <p className={`text-sm ${dark ? 'text-slate-400' : 'text-slate-600'}`}>
                Watch the priority queue process nodes and relax edges to find shortest paths.
              </p>
            </div>
            <div className="flex gap-2 flex-wrap">
              {PRESETS.map(p => (
                <button key={p.key} onClick={() => setConfig(DijkstraEngine.generate(p.key))}
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold ${dark ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}>
                  {p.label}
                </button>
              ))}
            </div>
          </div>
        </motion.div>

        <div className="grid lg:grid-cols-[1fr,340px] gap-5">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            className={`rounded-3xl border p-5 shadow-xl ${dark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-white/85'}`}>
            {/* Graph */}
            <div className={`rounded-2xl border p-4 ${dark ? 'border-white/5 bg-slate-950/60' : 'border-slate-100 bg-slate-50'}`}>
              <svg viewBox="0 0 340 300" className="w-full max-w-lg mx-auto">
                {/* Edges */}
                {config.edges.map(([u, v, w], i) => {
                  const from = nodePositions[u], to = nodePositions[v];
                  if (!from || !to) return null;
                  const isActive = current?.activeEdge?.[0] === u && current?.activeEdge?.[1] === v ||
                                   current?.activeEdge?.[0] === v && current?.activeEdge?.[1] === u;
                  const mx = (from.x + to.x) / 2, my = (from.y + to.y) / 2;
                  return (
                    <g key={i}>
                      <line x1={from.x} y1={from.y} x2={to.x} y2={to.y}
                        stroke={isActive ? '#2dd4bf' : dark ? '#475569' : '#cbd5e1'}
                        strokeWidth={isActive ? 3 : 1.5} opacity={isActive ? 1 : 0.5} />
                      <text x={mx} y={my - 6} textAnchor="middle" fontSize="10"
                        fill={isActive ? '#2dd4bf' : dark ? '#94a3b8' : '#64748b'} fontWeight="bold">{w}</text>
                    </g>
                  );
                })}
                {/* Nodes */}
                {nodePositions.map((pos, i) => {
                  const visited = current?.visited?.[i];
                  const isCurrent = current?.current === i;
                  const dist = current?.distances?.[i];
                  return (
                    <g key={i}>
                      <circle cx={pos.x} cy={pos.y} r={20}
                        fill={isCurrent ? '#14b8a6' : visited ? (dark ? '#0d9488' : '#5eead4') : dark ? '#1e293b' : '#f1f5f9'}
                        stroke={isCurrent ? '#2dd4bf' : visited ? '#14b8a6' : dark ? '#475569' : '#cbd5e1'}
                        strokeWidth={isCurrent ? 3 : 2} />
                      <text x={pos.x} y={pos.y - 2} textAnchor="middle" dominantBaseline="central"
                        fill={isCurrent || visited ? 'white' : dark ? '#94a3b8' : '#475569'}
                        fontSize="13" fontWeight="bold">{i}</text>
                      <text x={pos.x} y={pos.y + 14} textAnchor="middle"
                        fill={dark ? '#67e8f9' : '#0891b2'} fontSize="9" fontWeight="bold">
                        {dist === Infinity ? '∞' : dist}
                      </text>
                    </g>
                  );
                })}
              </svg>
            </div>

            {/* Controls */}
            <div className="flex items-center gap-3 mt-4">
              <button onClick={() => { setStepIdx(0); setPlaying(false); }}
                className={`p-2 rounded-xl ${dark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                <RotateCcw className="h-4 w-4" />
              </button>
              <button onClick={() => setPlaying(!playing)}
                className="p-2.5 rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 text-white shadow-lg">
                {playing ? <Pause className="h-4 w-4" /> : <Play className="h-4 w-4" />}
              </button>
              <button onClick={() => setStepIdx(i => Math.min(steps.length - 1, i + 1))}
                className={`p-2 rounded-xl ${dark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                <SkipForward className="h-4 w-4" />
              </button>
              <div className="flex-1" />
              <input type="range" min={200} max={1500} step={100} value={1700 - speed}
                onChange={e => setSpeed(1700 - Number(e.target.value))} className="w-24 accent-teal-500" />
              <span className={`text-xs font-mono ${dark ? 'text-slate-500' : 'text-slate-400'}`}>{stepIdx + 1}/{steps.length}</span>
            </div>
            <div className={`h-1.5 rounded-full mt-3 ${dark ? 'bg-slate-800' : 'bg-slate-200'}`}>
              <motion.div className="h-full rounded-full bg-gradient-to-r from-teal-500 to-cyan-500"
                animate={{ width: `${steps.length > 1 ? (stepIdx / (steps.length - 1)) * 100 : 0}%` }} />
            </div>
            <AnimatePresence mode="wait">
              <motion.div key={stepIdx} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className={`mt-3 p-3 rounded-xl text-sm ${dark ? 'bg-teal-950/30 text-teal-200 border border-teal-800/30' : 'bg-teal-50 text-teal-800 border border-teal-200'}`}>
                {current?.explanation}
              </motion.div>
            </AnimatePresence>
          </motion.div>

          {/* Sidebar */}
          <div className="flex flex-col gap-4">
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
              className={`rounded-2xl border p-4 ${dark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-white/85'}`}>
              <h3 className={`text-xs font-semibold uppercase tracking-[0.2em] mb-3 ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                <Zap className="h-3 w-3 inline mr-1" /> Distances
              </h3>
              <div className="space-y-1.5">
                {(current?.distances ?? []).map((d, i) => (
                  <div key={i} className={`flex justify-between items-center px-2 py-1 rounded-lg text-xs ${
                    current?.current === i ? (dark ? 'bg-teal-900/30 text-teal-300' : 'bg-teal-50 text-teal-700') :
                    current?.visited?.[i] ? (dark ? 'bg-slate-800/60 text-slate-300' : 'bg-slate-100 text-slate-600') :
                    dark ? 'text-slate-500' : 'text-slate-400'}`}>
                    <span className="font-bold">Node {i}</span>
                    <span className="font-mono">{d === Infinity ? '∞' : d}</span>
                  </div>
                ))}
              </div>
            </motion.div>

            <div className={`rounded-2xl border p-4 ${dark ? 'border-white/10 bg-slate-900/70' : 'border-slate-200 bg-white/85'}`}>
              <h3 className={`text-xs font-semibold uppercase tracking-[0.2em] mb-2 ${dark ? 'text-slate-400' : 'text-slate-500'}`}>
                <Sparkles className="h-3 w-3 inline mr-1" /> How It Works
              </h3>
              <p className={`text-xs leading-relaxed ${dark ? 'text-slate-400' : 'text-slate-600'}`}>
                Dijkstra uses a min-heap to always process the nearest unvisited node. For each node, relax all outgoing edges. Works for non-negative weights only. Time: O((V+E) log V).
              </p>
            </div>
          </div>
        </div>

        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
          <CodeDisplay code={code} language={language} title="Dijkstra's Algorithm — Shortest Path" currentLine={current?.highlightLine} />
        </motion.div>
      </div>
    </div>
  );
};

export default Dijkstra;
